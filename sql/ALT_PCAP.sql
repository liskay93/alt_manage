...
--   CURR_ID(USD/KRW 통화), CURR_TYP(CD/CP), RPRT_NM('AS Reported by Fund' / 'AS Reported by GCM')
-- 규칙
--   최신 제공일(WRK_DT 최대) 한 벌, GCM 보고 기준만 쓴다
--   통화 유형(CD/CP)은 SQL 에서 고르지 않고 long 으로 모두 돌려주며 processor 가 원화·펀드통화를 고른다
-- 돌려주는 열(대문자): PROV_DT, WRK_DT(=PCAP_DATE), FUND_CD, CURR_ID, CURR_TYP,
--                     COMMIT_AMT, FUNDED_AMT(양수로 뒤집음), DISTRB_AMT, NAV_AMT
-- 확인 중: 금액 단위(원/천/백만 → 억원·백만으로 나누는 값), CD/CP 의 뜻, PCAP_DATE 가 문자인지 DATE 인지, 이력 시작 시점
