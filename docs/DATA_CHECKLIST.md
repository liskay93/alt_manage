...
| # | 테이블 | 필수 | 용도 | 상태 |
|---|---|---|---|---|
| 1 | 펀드 마스터 | 필수 | 펀드 식별, 자산군·통화 분류, 펀드 기여도 카드 | ✅ 완료 — FEIAI0488NTA (펀드코드·빈티지·통화) + MAAMC0101DTM_CW01 (FUND_CD, ATVT_PGM_FUND_CD → 자산군·세부 분류), 펀드명은 FEIAI0488NTA.DEAL_NM. sql/ALT_Fund.sql 완성 (FUND_CD 표기 확인 필요) |
...
| 5 | 연도별 목표 | 필수 | 네 지표의 목표·달성률·잔여, 목표 점선 | ✅ 완료 — 엑셀 양식 data/ALT_Target.xlsx ('목표' 시트: 연도·자산군·약정·집행·회수·순증). 내부 숫자는 형님이 내부망에서 입력 |
...
DB 가 아니라 엑셀 양식 `data/ALT_Target.xlsx` 의 '목표' 시트로 받습니다 (내부 정보라 시연용 임의 숫자를 넣어 두었고, 내부망에서 숫자만 바꿉니다).

| 열 | 필수 | 설명 |
|---|---|---|
| 연도 | 필수 | 2021~2026 |
| 자산군 | 필수 | 사모 / 부동산 / 인프라 (화면에서는 사모 → 사모벤처) |
| 약정 | 필수 | 억원, 노란 칸 |
| 집행 | 필수 | 억원, 노란 칸 |
| 회수 | 필수 | 억원, 노란 칸. 대시보드의 분배 |
| 순증 | 자동 | 집행 − 회수 수식 |
